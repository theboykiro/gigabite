...
every time. Stored locally, backed up by file sync, never pushed to GitHub or anywhere
web-facing.
...
| Core protocol (`core.md`) | `~/.core/` local | Cloud sync | **Never** |
| Project knowledge / notes | `~/.knowledge/` local | Cloud sync | **Never** |
...
notes — stays local, cloud-synced, never web-facing. Credentials are held in the OS
